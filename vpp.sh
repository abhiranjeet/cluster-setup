...
sudo docker restart ns-client-2
ping -c 4 192.168.122.90
sudo docker exec -it ns-client-2 vppctl delete interface memif memif1/0
sudo docker exec -it ns-client-2 vppctl delete interface memif memif1/1
sudo docker exec -it ns-client-2 vppctl create memif socket id 1 filename /tmp/memif1.sock
sudo docker exec -it ns-client-2 vppctl create interface memif id 0 socket-id 1 ring-size 1024 buffer-size 2048 slave rx-queues 1 tx-queues 1
sudo docker exec -it ns-client-2 vppctl create interface memif id 1 socket-id 1 ring-size 1024 buffer-size 2048 slave rx-queues 1 tx-queues 1
sudo docker exec -it ns-client-2 vppctl set interface state memif1/0 up
sudo docker exec -it ns-client-2 vppctl set interface state memif1/1 up
sudo docker exec -it ns-client-2 vppctl set interface l2 xconnect memif1/0 memif1/1
sudo docker exec -it ns-client-2 vppctl set interface l2 xconnect memif1/1 memif1/0
figlet "Done"
